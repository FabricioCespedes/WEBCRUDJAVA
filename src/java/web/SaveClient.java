...
public class SaveClient extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            client.setDirection(request.getParameter("txtDireccion"));
            if (client.getId_client() > 0) {
                if (logic.getOne(" id_client = " + client.getId_client()).isExist()) {
                    resultado = logic.update(client);
                    msj = logic.getMessage();
                } else {
                    msj = "No se logro modificar por que el cliente ya no existe";
                }
            } else {
                resultado = logic.insert(client);
                msj = logic.getMessage();
            }
